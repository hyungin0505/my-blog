...
`CHECK_FILE`에서는 `fp`의 `_IO_file_flags`의 매직 마스크를 검사하여 제대로 된 파일 디스크립터인지를 검사하는데 이와 관련된 내용은 후술하겠다  
아무튼 `fp`가 파일 디스크립터가 아니면 0을 리턴하게 된다  
...
pwndbg로 `fopen()`을 호출한 후 RAX 레지스터에는 `fopen()`의 리턴값인 파일 디스크립터가 들어있다   
해당 파일 디스크립터를 `_IO_FILE_plus` 구조체로 출력하면 `_IO_FILE` 구조체와 `vtable`의 주소가 나온다  
<br>
...
pwntools에 있는 FilStr을 사용하려 했으나 일부분만 덮어야 해서 보류한다  
쓰려면 잘라서 쓸 수 있겠지만 굳이..  
<br>
